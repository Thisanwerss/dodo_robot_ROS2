...
#define Motor_id uint32_t
    ...
    // 原来的 CAN_Receive_Frame 保留类型定义（兼容），但在 SocketCAN 版本中不再实际使用
    typedef struct
    {
        uint8_t FrameHeader;
    ...
    {
        uint8_t FrameHeader[2] = {0x55, 0xAA}; // 帧头（在 SocketCAN 版本中不会真正发出去，仅保留结构兼容）
 ...
 *
 * 旧版本：使用USB转CAN + Linux 虚拟串口
 * 新版本：使用 SocketCAN + CanPort，直接操作 can0 接口
 */
    class Motor_Control
    {
    public:
        ...
        * 默认 can 接口为 "can0"
        */
        Motor_Control(CanPort::SharedPtr can = nullptr): can_(std::move(can))
        {
            if (can_ == nullptr) {
                // default can interface
                can_ = std::make_shared<CanPort>("can0");
            }
        }
        ...
        */
        ...
        /*
         * @brief refresh motor status 刷新电机状态（请求状态）
         * @param motor object 电机对象
         */
        void refresh_motor_status(const Motor& motor)
        {
            ...
            can_->send(send_data.canId, send_data.data, send_data.len);
            this->receive();
        }
        /*
        ...
        */
        ...
        void control_pos_force(Motor &DM_Motor,float pos, uint16_t vel, uint16_t i)
        {
            ...
            std::array<uint8_t, 8> data_buf{};
            std::memcpy(data_buf.data(), &pos, sizeof(float));
            std::memcpy(data_buf.data() + 4, &vel, sizeof(uint16_t));
            std::memcpy(data_buf.data() + 6, &i, sizeof(uint16_t));
            id=id+POSI_MODE;
            send_data.modify(id, data_buf.data());
            can_->send(send_data.canId, send_data.data, send_data.len);
            this->receive();
        ...
        /*
         * @description 函数库内部调用,用于解算电机can线返回的电机状态参数
         * 在 SocketCAN 版本中，直接从 can0 读取 8 字节 CAN 数据帧
         */
        void receive()
        {
            uint32_t canId = 0;
            uint8_t data[8] = {0};
            uint8_t len = 0;
            ssize_t nbytes = can_->recv(canId, data, len);
            if (nbytes <= 0 || len < 6)
            {
                return;
            }
            ...
            static auto uint_to_float = [](uint16_t x, float xmin, float xmax, uint8_t bits) -> float {
                ...
                float data_f = data_norm * span + xmin;
                return data_f;
            };
        ...
        /*
         * @description 接收寄存器参数回复帧（用于 read_motor_param / change_motor_param 等）
         * 这里假设电机通过 CAN 直接返回 8 字节帧，格式与原先 canData 相同：
         * data[0..1] = ID，data[2] = 0x33/0x55，data[3] = RID，data[4..7] = 数据
         */
        void receive_param()
        {
            uint32_t canId = 0;
        ...
        bool change_motor_param(Motor &DM_Motor,uint8_t RID,float data)
        {
            ...
            for(uint8_t i =0;i<max_retries;i++)
            {
                ...
                if (motors[DM_Motor.GetSlaveId()]->is_have_param(RID))
                {
                    ...
                    {
                        return std::fabs(motors[DM_Motor.GetSlaveId()]->get_param_as_float(RID) - data)<0.1f;
                    }
                }
            }
            return false;
    ...
    private:
        ...
        CanPort::SharedPtr can_;   // SocketCAN 端口
        can_send_frame send_data;  // 发送数据缓存（只用 canId + data[8]）
    };
